...
import (
	"errors"
	"fmt"
	"github.com/rjman-self/Platdot/chains/platdot"
	"net/http"
	"os"
...
var accountCommand = cli.Command{
	...
	Description: "The accounts command is used to manage the bridge keystore.\n" +
		"\tTo generate a new account (key type generated is determined on the flag passed in): platdot accounts generate\n" +
		"\tTo import a keystore file: platdot accounts import path/to/file\n" +
		"\tTo import a geth keystore file: platdot accounts import --ethereum path/to/file\n" +
		"\tTo import a private key file: platdot accounts import --privateKey private_key\n" +
		"\tTo list keys: platdot accounts list",
...
func run(ctx *cli.Context) error {
	...
	for _, chain := range cfg.Chains {
		...
		if chain.Type == "ethereum" {
			newChain, err = platdot.InitializeChain(chainConfig, logger, sysErr, m)
